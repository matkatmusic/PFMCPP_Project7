...
{
    std::cout << getName() << " is taking " << damage << " damage!"
              << std::endl;
    if (damage < armor)
    {
        armor -= damage;
...
void Character::attackInternal(Character& other)
{
    if (other.hitPoints <= 0)
    {
        ...
        When you defeat another Character:
            ...
            c) the initial value of your stats is updated to reflect this
        boosted stat for the next time you defeat another character.
      */
        hitPoints = static_cast<int>(*initialHitPoints * 1.1);
